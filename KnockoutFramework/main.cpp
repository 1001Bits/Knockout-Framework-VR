...
// The original 1.4 Papyrus maintenance script treats PluginInfo::version as
// the framework API version and refuses to initialize when it is below 10400.
// Keep that compatibility value independent from the VR port's public release
// version (currently 1.0), which is reported in the log and PE resource.
constexpr UInt32 kPapyrusPluginVersion = 10400;
...
extern "C" {
	bool F4SEPlugin_Query(const F4SEInterface * f4se, PluginInfo * info) {
		...
		_MESSAGE("%s library v%d.%d.%d (based on Knockout Framework 1.4.0, F4SE compatibility version %u) - Loaded", PLUGIN_NAME, PLUGIN_VERSION_MAJOR, PLUGIN_VERSION_MINOR, PLUGIN_VERSION_BUILD, kPapyrusPluginVersion);

		info->infoVersion = PluginInfo::kInfoVersion;
		info->name = FILE_NAME;
		info->version = kPapyrusPluginVersion;
